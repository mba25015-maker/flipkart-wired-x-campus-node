...
where AOV enters. So the deck's climax and the client's own hurdle are two legs of one identity,
...
def report():
    ...
    print(f"    AOV for ROCE = {ROCE_HURDLE:.0%}  (the client's hurdle)   Rs{AOV_HURDLE:,.0f}"
    ...
    print(f"    >>> The hurdle is {'REACHABLE' if HURDLE_INSIDE_CEILING else 'NOT reachable'} inside "
          f"management's own stated non-grocery ceiling.")
    print()
    ...
    print(f"    Rs{CE_BASE/1e5:,.1f} lakh of capital. The client manages to {ROCE_HURDLE:.0%}. The gap is")
    ...
    print(f"    financials slide, and it is inside the ceiling management has already stated.")
