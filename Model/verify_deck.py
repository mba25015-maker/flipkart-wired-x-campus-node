...
must("S7  Swiggy disclosed reference range",     f"{BK.NONGROCERY_CEILING_LO:.0f}–{BK.NONGROCERY_CEILING:.0f}%")
...
must("S4  external ROCE benchmark",               f"{RC.ROCE_HURDLE:.0%}")
must("S4  AOV at the benchmark",                  f"{RC.AOV_HURDLE:,.0f}")
must("S4  AOV at breakeven, ROCE basis",         f"{RC.AOV_BREAKEVEN:,.0f}")
must("S4  benchmark premium",                    f"{RC.HURDLE_PREMIUM:,.0f}")
must("S4  non-grocery share benchmark implies",  f"{RC.HURDLE_NONGROCERY_SHARE:.1f}%")
...
must("S7  IRR at the benchmark AOV",             f"{RC.irr(RC.AOV_HURDLE):.1%}")
must("S7  ROCE at a 30% basket",                 f"{RC.roce(RC.AOV_UP):.1%}")
must("S7  post-tax benchmark AOV",               f"{RC.AOV_HURDLE_POSTTAX:,.0f}")
